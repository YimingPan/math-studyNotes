...
高斯消元是把一个矩阵变成一个行阶梯阵（row echelon form）。（有谁想不开搞个列阶梯阵？）约当消元则进一步把行阶梯阵变成最简行阶梯阵（reduced row echelon form），这样做有什么意义呢？比如我们要解方程组$Ax=b$，当我们把[A b]化为最简行阶梯阵后，原来b这一列就变成解向量了，方便程序直接输出。其实这就是我们在草稿纸上通过回代把变量一个个解出来的过程。哦，忘了说，这么做的前提是这个方程组有唯一解，也即A可逆。

具体的操作过程我就不说了，我想在这里补充的是一些英文术语，可能会让你觉得无聊。我之所以要补充，是因为中文教材貌似都不翻译它们，而这些术语对于后面某些概念的表述有简化作用。

**Pivot Variable & Pivot Column**
Pivot Variable是指阶梯阵里每行的第一个元素，它所在的那一列就叫Pivot Column。
...
* 求逆矩阵。再次祭出消元大法。
* **可逆矩阵的几个充分必要条件**。这是线性代数最重要的一串等价命题之一。
* 这一部分书上出现过“伴随矩阵”的概念，国内的教材通常表示成$A^*$。这东西对理论推导有价值，可以解决一些证明问题，对运算是没什么帮助的，毕竟没人会自找麻烦用它去求逆矩阵。

这里再补充一些术语，因为英文教材的terminology和中文差别实在太大了，写书的人完全是造了新词，而不是翻译人家的名词。
